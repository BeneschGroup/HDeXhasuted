...
Averages the relative updtake of Deuteros statistically siginificant peptides.
Generates PyMOL script to colour structure based on b-factors
Put all files and this script in one folder
"""
import numpy as np
...
protein_length = 244
protein_name = '6s7e'
file_path = '250115_PCO4_Cluster_Table_FilteredPeptides.csv' #file taken from deuteros datatable filtered peptides
...
for exposure in unique_exposure:
    ...
    for aa_num in range(1, protein_length + 1):
        ...
        relevant_peptides = relevant_peptides[(relevant_peptides['Start'] <= aa_num-1) & (relevant_peptides['End'] >= aa_num)] #Removes first AA from each peptide
